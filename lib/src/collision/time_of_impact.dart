...
class SeparationFunction {
  ...
  num initialize(SimplexCache cache, DistanceProxy argProxyA, Sweep
    ...
    if (count == 1) {
      ...
      localPoint.setFrom(localPointB1).add(localPointB2).scale(0.5);
      Transform.mulToOut(xfb, localPoint, pointB);

      localPointA.setFrom(proxyA.vertices[cache.indexA[0]]);
      Transform.mulToOut(xfa, localPointA, pointA);

      temp.setFrom(pointA).sub(pointB);
      ...
      temp.setFrom(localPointA2).sub(localPointA1);
      cross(temp, 1.0, axis);
      axis.normalize();

      xfa.rotation.transformed(axis, normal);

      localPoint.setFrom(localPointA1).add(localPointA2).scale(0.5);
