...
$$.DominoTest = {"":
 ...
 initialize$0: function() {
  ...
  bd.type = 2;
  for (i = 0; i < 4; ++i)
    for (var t3 = 7.3 + 5 * i, t4 = i === 2, t5 = i === 3, j = 0; j < 25; ++j) {
      fd.friction = 0.5;
      bd.position = $.Vector$(-14.75 + j * 1.2291666666666667, t3);
      if (t4 && j === 0) {
        bd.angle = -0.1;
        var t6 = bd.position;
        t6.set$x($.add(t6.get$x(), 0.1));
      } else if (t5 && j === 24) {
        bd.angle = 0.1;
        t6 = bd.position;
        t6.set$x($.sub(t6.get$x(), 0.1));
      } else
        bd.angle = 0;
      var myBody = t1.createBody$1(bd);
...
$.DominoTest$ = function() {
  var t1 = new $.DominoTest($.ListImplementation_List(null, 'Body'), null, null, null, null, null, null, null, null, null, 10, $.throwNoSuchMethod('', 'constructorClosure: () => String from Function \'slowToString\':.', []));
