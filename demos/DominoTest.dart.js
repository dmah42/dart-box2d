...
$$.DominoTest = {"":
 ...
 initialize$0: function() {
  ...
  bd = $.BodyDef$();
  bd.type = 2;
  for (var t3 = 25 - 1, t4 = 29.5 / t3, i = 0; i < 4; ++i) {
    for (var t5 = 7.3 + 5 * i, t6 = i === 2, t7 = i === 3, j = 0; j < 25; ++j) {
      fd.friction = 0.5;
      bd.position = $.Vector$(-14.75 + j * t4, t5);
      if (t6 && j === 0) {
        bd.angle = -0.1;
        var t8 = bd.position;
        t8.set$x($.add(t8.get$x(), 0.1));
      } else {
        if (t7 && j === t3) {
          bd.angle = 0.1;
          t8 = bd.position;
          t8.set$x($.sub(t8.get$x(), 0.1));
        } else bd.angle = 0;
      }
