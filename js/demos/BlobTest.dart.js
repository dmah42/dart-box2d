...
PolygonShape.prototype.setAsBoxWithCenterAndAngle = function(hx, hy, center, angle) {
  ...
  for (var i = (0);
   i < this.vertexCount; ++i) {
    Transform.mulToOut(xf, this.vertices[i], this.vertices[i]);
    Matrix22.mulMatrixAndVectorToOut(xf.rotation, this.normals[i], this.normals[i]);
  }
}
...
CanvasDraw.prototype.drawSolidPolygon = function(vertices, vertexCount, color) {
  ...
  for (var i = (0);
   i < vertexCount; i++) {
    this.getWorldToScreenToOut(vertices[i], vertices[i]);
  }
  this.ctx.beginPath();
